...
do  echo "Generating dependencies for $src"
	...
	\$(RUN_CMD_CC)\$(LIBTOOL) --mode=compile \$(CC) \$(CFLAGS) \$(EXTRA_CFLAGS) -c $src  -o \$@
...
__EOF__
	...
	\$(RUN_CMD_CC)\$(LIBTOOL) --tag=CC --mode=compile \$(auxdir)/strip_fPIC.sh \$(NASM) -I\$(srcdir)/src/hermes/ $src -o \$@
